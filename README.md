...
- `/resume` 采用 OpenCode 风格的历史记录选择器：可直接输入过滤，按 Today/Yesterday/日期分组，行尾显示工作区目录，按 `i` 或 `?`（非过滤时）可查看 ID/路径详情。
...
dsh plugin --profile safe add github:aorucshiea/dsh-safe-tui#v0.4.10
```
