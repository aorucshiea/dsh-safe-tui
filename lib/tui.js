...
export class TUI {
  ...
  addEvent(event) {
    ...
    else if (event.type === "turn/end") this._pushItem({ kind: "text", text: `${dim(`· turn ended: ${event.data?.reason?.kind ?? "unknown"}`)}` });
  }
    ...
    for (const item of this.items) {
      if (item.kind === "text") {
        ...
        if (this.expandedBlocks.has(item)) {
          ...
          const summary = summarizeText(item.text, 90);
        ...
        if (this.expandedBlocks.has(item)) {
          ...
          const header = summarizeText(item.header ?? item.text ?? "", 90);
  ...
  _footer() {
    const lines = [
      ...
      `  ${dim("safe mode · only minimal / standard presets · no user plugins · type / for commands · ctrl+o expand/collapse details")}`
    ];
  ...
  _handleMouse(button, x, y, kind) {
    ...
    if (button === 64) {
      this._scrollHistory(3);
    } else if (button === 65) {
      this._scrollHistory(-3);
    }
    // Click handling can be added later; wheel scrolling is the important part.
    return;
  }
  ...
  _scrollHistory(delta) {
    ...
    const palette = this.picker
      ...
      : this.paletteOpen && this._paletteItems().length > 0
        ? this._paletteLines(this._paletteItems(), cols)
        : [];
    const available = Math.max(1, rows - header.length - footer.length - status.length - 3 - palette.length);
    const maxScroll = Math.max(0, this._allLines().length - available);
...
function summarizeText(text, max = 90) {
