...
const COMMANDS = [
  { command: "/list", description: "List titled conversations" },
  { command: "/list all", description: "List every saved session including untitled" },
  { command: "/resume <id>", description: "Resume a session (inherits history)" },
  { command: "/resume all", description: "Resume from every saved session including untitled" },
...
export class TUI {
  ...
  render() {
    ...
    // Redraw without clearing the whole screen to avoid flicker on mouse wheel.
  ...
  openPicker({ title, items, onSelect, filterable = false } = {}) {
  ...
  _pickerLines(cols) {
    ...
    if (this.picker?.filterable) {
      ...
      const visibleFilter = filterText.replace(/\u001b\[[0-9;]*m/g, "").length;
    ...
    for (let offset = 0; offset < items.length; offset += 1) {
      ...
      if (item.category && item.category !== lastCategory) {
        ...
        const visibleCat = cat.replace(/\u001b\[[0-9;]*m/g, "").length;
      ...
      if (item.group) {
        const inner = `  ${dim(label)}  `;
        const visibleInner = inner.replace(/\u001b\[[0-9;]*m/g, "").length;
        const pad = Math.max(0, boxWidth - 2 - visibleInner);
        lines.push(`  ${gray("│")}${inner}${" ".repeat(pad)}${gray("│")}`);
        continue;
      }
      const footer = item.footer ? String(item.footer) : "";
      const innerWidth = boxWidth - 2;
      const footerWidth = Math.min(footer.length, Math.max(6, Math.floor(innerWidth / 3)));
      const footerText = footer.length > footerWidth ? footer.slice(0, Math.max(1, footerWidth - 1)) + "…" : footer;
      const leftWidth = Math.max(4, innerWidth - footerText.length - 2);
      const left = label.length > leftWidth ? label.slice(0, Math.max(1, leftWidth - 1)) + "…" : label;
      const spaces = " ".repeat(Math.max(1, innerWidth - left.length - footerText.length - 1));
      const inner = ` ${left}${spaces}${footerText} `;
      if (selected) {
    ...
    if (this.picker?.showDetails) {
      ...
      const shown = detail.length > maxDetail - 1 ? detail.slice(0, maxDetail - 2) + "…" : detail;
      lines.push(`  ${cyan("ℹ")} ${gray(shown)}`);
    }
    const hint = this.picker?.filterable ? "type to filter · ↑/↓ select · enter choose · esc cancel" : "↑/↓ select · enter choose · i/? details · esc cancel";
