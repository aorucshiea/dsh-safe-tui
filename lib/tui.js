...
export class TUI {
  ...
  render() {
    ...
    if (overlay) {
      // Clean overlay: only the picker / command palette, no history behind it.
      if (palette.length > 0) lines.push(...palette);
      while (lines.length < headerLines.length + available + palette.length) lines.push("");
    } else {
  ...
  _pickerLines(cols) {
    ...
    lines.push(`  ${gray("↑/↓ select · enter choose · i/? details · esc cancel")}`);
    return lines;
  }
