...
export class TUI {
  ...
  _pickerLines(cols) {
    ...
    for (let offset = 0; offset < items.length; offset += 1) {
      ...
      if (detail.length > maxDetail - 1) detail = detail.slice(0, maxDetail - 2) + "…";
      ...
      if (detail !== "") {
        const detailInner = `   ${dim(detail)} `;
        const visibleDetail = detailInner.replace(/\u001b\[[0-9;]*m/g, "").length;
        const detailPad = Math.max(0, boxWidth - 2 - visibleDetail);
        if (selected) {
          lines.push(`  ${gray("│")}${ESC}48;5;237m${detailInner}${" ".repeat(detailPad)}${reset()}${gray("│")}`);
        } else {
          lines.push(`  ${gray("│")}${detailInner}${" ".repeat(detailPad)}${gray("│")}`);
        }
      }
    }
