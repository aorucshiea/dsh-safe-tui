...
export class TUI {
  ...
  addEvent(event) {
    if (event.type === "user/message") {
      ...
      if (text) this.println(`${cyan("you")}  ${text}`);
      return;
    }
    if (event.type === "assistant/message") {
      for (const block of messageBlocks(event.data?.message)) {
        if (block.kind === "text") this.println(`${green("dsh")}  ${block.text}`);
        else if (block.kind === "reasoning") this.println(`  ${dim("💭 " + block.text)}`);
        else if (block.kind === "tool") this.println(`  ${yellow("⚙ " + block.text)}`);
        else if (block.kind === "tool-result") this.println(`  ${gray("↳ result: " + block.text)}`);
      }
      return;
    }
    if (event.type === "turn/start") this.println(`${dim("· turn started")}`);
    else if (event.type === "turn/end") this.println(`${dim(`· turn ended: ${event.data?.reason?.kind ?? "unknown"}`)}`);
  }
  ...
  render() {
    ...
    const maxScroll = Math.max(0, this.lines.length - available);
