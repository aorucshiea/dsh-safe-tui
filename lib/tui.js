...
export class TUI {
  ...
  _pickerLines(cols) {
    ...
    for (let offset = 0; offset < items.length; offset += 1) {
      ...
      const leftMax = Math.max(4, innerWidth - footerText.length - 1);
      const left = label.length > leftMax ? label.slice(0, Math.max(1, leftMax - 1)) + "…" : label;
      const spaces = " ".repeat(Math.max(1, innerWidth - left.length - footerText.length));
