// DeepSeek Harness safe-mode recovery console.
//
// This plugin runs on top of dsh-base with NO web UI and NO user plugin
// bundles. It intentionally only exposes the shipped `minimal` and `standard`
// presets (user-authored presets are hidden), never loads the web client, and
// can repair the web profile's fragile patched files from pristine copies.
...
import { TUI, cyan, green, gray, dim, red, yellow, blue, bold } from "./tui.js";

export const name = "safe-console";
export const inject = [
  "cmdlineArgs",
  "loader",
  "agents",
...
const ALLOWED_PRESETS = ["minimal", "standard"];
...
async function resumeAgent(ctx, sessionId, out = console.log) {
  ...
  printHistoryTail(agent, 8, out);
  return agent;
}

async function listSessions(ctx, out = console.log) {
  const headers = await ctx.get("sessionPersistence").list();
  out("\nsaved sessions:");
  if (headers.length === 0) {
    out("  (none)");
...
async function runREPL(ctx, initialSessionId) {
  ...
  if (!initialSessionId) {
    console.log("\n[safe] recovery console ready. /help for commands, /resume <id> to continue history.");
  }
  ...
  /preset <id>          switch preset: ${ALLOWED_PRESETS.join(", ")} (blank session only)
...
`);
    ...
    if (input === "/preset") {
      ...
      if (state.agent) {
        ...
        console.log(`current session has history: ${hasHistory ? "yes (locked)" : "no"}`);
      }
      safePrompt();
      return;
    }
    if (input.startsWith("/preset ")) {
      ...
      if (!state.agent) {
        console.log("[safe] no current session; the next new session will default to standard/minimal only.");
        safePrompt();
        return;
      }
      const hasHistory = state.agent.session.events.some((event) => event.type === "turn/start");
      if (hasHistory) {
        console.error(`[safe] cannot switch preset on a session with existing turns (locked by history)`);
        safePrompt();
        return;
      }
      try {
...
async function runTUI(ctx, initialSessionId) {
  ...
  const tui = new TUI({
    ...
    onSubmit: async (raw) => {
      ...
      if (input.startsWith("/preset ")) {
        ...
        if (!state.agent) {
          say("[safe] no current session; the next new session will default to standard/minimal only.");
          return;
        }
        const hasHistory = state.agent.session.events.some((event) => event.type === "turn/start");
        if (hasHistory) {
          say(`${red("error")} cannot switch preset on a session with existing turns (locked by history)`);
          return;
        }
        try {
          const preset = await ctx.get("agentPresets").recompose(state.agent.ctx, id);
    ...
    tui.openPicker({
      ...
      onSelect: async (item) => {
        try {
          ...
          say(`[safe] default model set: ${item.value}`);
  ...
  const openSessionPicker = async () => {
    const headers = await ctx.get("sessionPersistence").list();
    if (headers.length === 0) {
      say(gray("[safe] no saved sessions"));
      return;
    }
    const items = headers.map((header) => ({
      label: `${header.id}  ·  ${header.cwd ?? "?"}  ·  ${header.agentPreset ?? "preset?"}`,
      value: header.id
    }));
    tui.openPicker({
...
CLI:
  dsh --profile safe                    interactive recovery console (TUI)
  dsh --profile safe "task text"        answer one task and exit
  dsh --profile safe --list             list saved sessions and exit
  dsh --profile safe --resume <id>      open interactive console on a saved session
...
TUI commands:
  /list /resume <id> /new /preset /preset <id>
  /models | /model   /models <id> | /model <id>   /add-provider   /providers
  /status /repair /check /help /quit

Safe mode loads only dsh-base + safe-console. No user plugins, no web UI,
no user-authored presets.`;
}
