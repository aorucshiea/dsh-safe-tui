...
async function runTUI(ctx, initialSessionId) {
  ...
  const openSessionPicker = async (includeAll = false) => {
    ...
    const items = [];
    for (const key of keys) {
      items.push({ label: `── ${key} ──`, value: "", group: true });
      const groupItems = groups.get(key).sort((a, b) => (Number(b.header.createdAt) || 0) - (Number(a.header.createdAt) || 0));
      for (const row of groupItems) {
        items.push({
          label: row.title !== "" ? row.title : row.header.id,
          value: row.header.id,
          detail: `${row.header.id} · ${row.header.cwd ?? "?"}${row.preset}`
        });
      }
    }
    if (items.length === 0) {
