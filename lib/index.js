...
export function apply(ctx) {
  ...
  // it must not start a terminal in the server process. In a safe/CLI profile,
  // the original Safe TUI continues to run normally.
  let webServer
  try { webServer = ctx.get("webServer") } catch { /* not available in safe CLI profile */ }
  if (webServer) {
    applyWebConsole(ctx)
    return
  }
